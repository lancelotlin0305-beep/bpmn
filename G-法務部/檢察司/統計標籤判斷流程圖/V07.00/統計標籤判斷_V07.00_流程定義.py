...
def build_pretrial():
    ...
    p.add("a_4",   "task",    "波特玩判讀五類統計欄位", 0, kind="system")
...
def build_pretrial_notes():
    ...
    p.add("c_4",   "task",    "波特玩判讀五類統計欄位", 0, kind="system")
    ...
    p.add("c_n3", "note",
          "波特玩判讀完個案欄位屬性後,將判讀結果拋轉至刑案管理系統之SFTP資料夾,後續由刑案管理系統讀取(RFP工項M3-04要求API整合,待釐清Q2)", 0)
...
def build_execution():
    ...
    p.add("b_2",   "task",    "波特玩判讀裁判書七項欄位", 0, kind="system")
...
def build_execution_notes():
    ...
    p.add("e_2",   "task",    "波特玩判讀裁判書七項欄位", 0, kind="system")
...
if __name__ == "__main__":
    ...
    emit_multi([build_pretrial(), build_execution(), build_pretrial_notes(), build_execution_notes()], "統計標籤判斷",
               outdir, version="V07.00", src=__file__,
               change="依RFP工項M3補充:偵查類補五類統計欄位、執行類補裁判書七項欄位任務名;介接頁加判讀規則(M3-01/M3-03)、裁判書API七欄位(M3-02)、API整合vs SFTP落差(M3-04)註解",
