...
if __name__ == "__main__":
    ...
    emit_multi([build_pretrial(), build_execution(), build_pretrial_notes(), build_execution_notes()], "統計標籤判斷",
               outdir, version="V07.02", src=__file__,
               change="註解移除待釐清編號(疑義僅留待釐清清單)",
               change_kind="文字", change_source="口頭指示")

    # ---- 後處理:跨系統自動介接連線標紅(builder 不支援線色,於輸出檔後製) ----
    import io, re
    RED = "#FF0000"
    vdir = os.path.join(outdir, "V07.02")
    ...
    dpath = os.path.join(vdir, "統計標籤判斷_V07.02.drawio")
    ...
    for pid, pairs in red_edges.items():
        spath = os.path.join(vdir, "%s_V07.02.svg" % pid)
    ...
    hpath = os.path.join(vdir, "統計標籤判斷_V07.02_檢視器.html")
