...
V07.01:依 RFP 工項拆解-v1(M3 裁判書資料智能判讀)補充——
  偵查類任務名補五類統計欄位(M3-01)、執行類補裁判書七項欄位(M3-02),
  介接頁加判讀規則引擎(M3-03)與 API 整合 vs SFTP 現行設計落差(M3-04)註解。
執行: python 統計標籤判斷_V07.01_流程定義.py [輸出資料夾]
"""
import sys, os
...
def build_pretrial():
    ...
    p.add("a_3",   "task",    "呼叫AI智慧輔助系統智慧判斷並傳送書類內文", 0, kind="system")
...
def build_pretrial_notes():
    ...
    p.add("c_3",   "task",    "呼叫AI智慧輔助系統智慧判斷並傳送書類內文", 0, kind="system")
    ...
    p.add("c_n2", "note",
          "由書類生成系統觸發呼叫AI智慧輔助系統,並傳遞起訴書/不起訴書之書類資訊", 0)
    p.add("c_n3", "note",
          "AI智慧輔助系統判讀完個案欄位屬性後,將判讀結果拋轉至刑案管理系統之SFTP資料夾,後續由刑案管理系統讀取(RFP工項M3-04要求API整合,待釐清Q2)", 0)
...
if __name__ == "__main__":
    ...
    emit_multi([build_pretrial(), build_execution(), build_pretrial_notes(), build_execution_notes()], "統計標籤判斷",
               outdir, version="V07.01", src=__file__,
               change="更名波特玩→AI智慧輔助系統;跨系統自動介接連線標紅(列印/儲存→呼叫判讀、拋轉→SFTP)",
               change_kind="文字", change_source="口頭指示")

    # ---- 後處理:跨系統自動介接連線標紅(builder 不支援線色,於輸出檔後製) ----
    import io, re
    RED = "#FF0000"
    vdir = os.path.join(outdir, "V07.01")
    red_edges = {  # pid → [(source, target, flow定義順序索引)]
        "統計標籤判斷_偵查類_工作流程圖": [("a_2", "a_3", 2), ("a_5", "a_sftp", 5)],
        "統計標籤判斷_執行類_工作流程圖": [("b_3", "b_sftp", 3)],
        "統計標籤判斷_偵查類_介接說明":   [("c_2", "c_3", 2), ("c_5", "c_sftp", 5)],
        "統計標籤判斷_執行類_介接說明":   [("e_3", "e_sftp", 3)],
    }

    # 1) .drawio:依 source/target 精準改 style(順序流原色 #3a4a59)
    dpath = os.path.join(vdir, "統計標籤判斷_V07.01.drawio")
    ...
    for pid, pairs in red_edges.items():
        spath = os.path.join(vdir, "%s_V07.01.svg" % pid)
    ...
    hpath = os.path.join(vdir, "統計標籤判斷_V07.01_檢視器.html")
