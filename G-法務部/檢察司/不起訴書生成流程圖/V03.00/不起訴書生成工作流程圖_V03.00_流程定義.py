...
def build():
    p = Proc("不起訴書生成工作流程圖", "檢察官不起訴書生成工作流程圖",
             ...
             bands=[("書類生成系統", ["s", "t0", "t1", "t1b"]),
                    ("波特玩系統", ["t2", "t3", "t4", "t5", "t5b", "t5c", "t6", "t7", "gw", "t8"]),
                    ("翰書系統", ["t9", "e"])])
    p.add("s",   "start",   "開始", 0)
    ...
    p.add("t5c", "task",    "確認適用法規條款", 0, kind="user")
...
if __name__ == "__main__":
    ...
    emit(build(), outdir, fmt="drawio", src=__file__,
         change="新增登錄/查詢個案/連線步驟、警詢筆錄、法規條款詢問與確認;匯入與指定類型對調", change_kind="結構", change_source="口頭指示")
