...
V08.00:新增頁3 現行工作流程——登錄書類生成系統查詢並點開個案,
  開啟漢書系統書類編輯介面,人工登打不起訴書後結束(無AI輔助)。
執行: python 不起訴書生成工作流程圖_V08.00_流程定義.py [輸出資料夾]
"""
import sys, os
...
def build_cur():
    ...
    p.flow("c_s", "c_t0"); p.flow("c_t0", "c_t1"); p.flow("c_t1", "c_t2")
    p.flow("c_t2", "c_t3"); p.flow("c_t3", "c_e")
    return p
...
if __name__ == "__main__":
    ...
    emit_multi([build("a_", "不起訴書生成工作流程圖", "檢察官不起訴書生成工作流程圖"),
                build("b_", "不起訴書生成工作流程圖_介接說明", "檢察官不起訴書生成工作流程圖(含介接說明)",
                      with_notes=True),
                build_cur()],
               "不起訴書生成工作流程圖", outdir, version="V08.00", src=__file__,
               change="新增頁3「現行工作流程(未導入AI)」:登錄書類生成系統查詢並"
                      "點開個案,開啟漢書系統書類編輯介面,人工登打不起訴書後結束",
