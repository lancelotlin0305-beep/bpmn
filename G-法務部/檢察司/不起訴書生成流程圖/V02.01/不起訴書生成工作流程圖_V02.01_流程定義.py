...
def build():
    p = Proc("不起訴書生成工作流程圖", "檢察官不起訴書生成工作流程圖",
             ["檢察官"], version="V02.01",
             bands=[("書類生成系統", ["s", "t1"]),
                    ("波特玩系統", ["t2", "t3", "t4", "t5", "t6", "t7", "gw", "t8"]),
                    ("翰書系統", ["t9", "e"])])
    p.add("s",   "start",   "開始", 0)
    p.add("t1",  "task",    "點開個案並連線波特玩", 0, kind="user")
    p.add("t2",  "task",    "明確指定書類類型(不起訴書)", 0, kind="user")
    p.add("t3",  "task",    "匯入移送書與偵訊筆錄", 0, kind="user")
    p.add("t4",  "task",    "波特玩詢問不起訴原因", 0, kind="system")
...
if __name__ == "__main__":
    ...
    emit(build(), outdir, fmt="drawio", src=__file__,
         change="更名:波特王→波特玩", change_kind="文字", change_source="口頭指示")
