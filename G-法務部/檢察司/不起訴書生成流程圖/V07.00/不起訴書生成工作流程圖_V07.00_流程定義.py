...
V07.00:依 RFP 工項拆解-v1(M1 檢察書類初稿生成)補充——
  卷證資料輸入(M1-03)、依案件類型自動選用範本(M1-01/M1-03)、
  自動Fine-tuning回饋(M1-05)、標準化介接匯入書類系統之需求註解(M1-06)。
執行: python 不起訴書生成工作流程圖_V07.00_流程定義.py [輸出資料夾]
"""
import sys, os
...
def build(prefix, pid, name, with_notes=False):
    ...
    p = Proc(pid, name, ["檢察官"],
             bands=[("書類生成系統", [q("s"), q("t0"), q("t1"), q("t1b")]),
                    ("波特玩系統", [q("t2"), q("t2b"), q("t3"), q("t4"), q("t5"),
    ...
    if with_notes:
        ...
        p.assoc(q("n2"), q("t8"))
        p.add(q("n3"), "note",
              "RFP工項M1-06:應以標準化介接直接匯入書類系統(現行人工匯入,待釐清Q2)", 0)
        p.assoc(q("n3"), q("e"))
...
if __name__ == "__main__":
    ...
    emit_multi([build("a_", "不起訴書生成工作流程圖", "檢察官不起訴書生成工作流程圖"),
                build("b_", "不起訴書生成工作流程圖_介接說明", "檢察官不起訴書生成工作流程圖(含介接說明)",
                      with_notes=True)],
               "不起訴書生成工作流程圖", outdir, version="V07.00", src=__file__,
               change="依RFP工項M1補充:卷證資料輸入、自動選用書類範本任務;介接頁加Fine-tuning回饋與標準化介接匯入需求註解",
