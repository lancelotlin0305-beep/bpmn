...
def build():
    ...
    p.add("t5c", "task",    "檢視適用法規條款", 0, kind="user")
    p.add("gw2", "gateway", "條款正確", 0)
    p.add("t5d", "task",    "修正適用法規條款", 0, kind="user")
    ...
    p.flow("t5c", "gw2")
    p.flow("gw2", "t6", "條款正確→是")
    p.flow("gw2", "t5d", "條款正確→否")
    p.flow("t5d", "t6")
    p.flow("t6", "t7")
    p.flow("t7", "gw")
    p.flow("gw", "t8", "需調整→是")
    p.flow("gw", "t9", "需調整→否")
    p.flow("t8", "t7", route="backLoop")
    p.flow("t9", "e")
    return p
...
if __name__ == "__main__":
    ...
    emit(build(), outdir, fmt="drawio", src=__file__,
         change="加調整後回檢視迴圈、條款正確閘道與修正任務、定稿工件;更名翰書→漢書", change_kind="結構", change_source="口頭指示")
