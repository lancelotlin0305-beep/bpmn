...
def build():
    ...
    p.add("s",     "start",   "開始",                          0)
    p.add("open",  "task",    "開啟個案管理系統",                0, kind="user")
    p.add("query", "task",    "於再犯風險評估智慧輔助系統\n查詢個案資料", 0, kind="user")
    p.add("load",  "task",    "導引至AI觀護助理輔助系統\n帶入個案資料(含獄政等資料)(M9)", 0, kind="system")
    ...
    p.add("wb",    "task",    "回填再犯風險評估智慧輔助系統\n存檔(M12-01)", 0, kind="system")
    p.add("sync",  "task",    "回存至個案管理系統(M12-01)",        0, kind="system")
...
if __name__ == "__main__":
    ...
    emit(build(), outdir, fmt="drawio", src=__file__,
         ...
         change="初版產出:依口述流程說明並對應工項拆解-v1.md(M5/M6/M7/M9/M12)",
         change_kind="初版", change_source="流程說明")
